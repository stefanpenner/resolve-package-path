...
function _findPackagePath(realFilePathCache: Cache, name: string, dir: string) {
  ...
  while (currPath !== fsRoot) {
    ...
    let endsWithNodeModules =
      path.basename(currPath).toLowerCase() === "node_modules";

    let filePath = path.join(
      currPath,
      endsWithNodeModules ? "" : "node_modules",
      name
    );
...
function resolvePackagePath(caches: CacheGroup, name?: string, dir?: string) {
  if (typeof name !== "string" || name.length === 0) {
    throw new TypeError(
      "resolvePackagePath: 'name' must be a non-zero-length string."
    );
  }
  ...
  while (
    _getRealDirectoryPath(caches.REAL_DIRECTORY_PATH, absoluteStart) === null
  ) {
