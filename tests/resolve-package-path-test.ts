...
describe("resolvePackagePath", function () {
  ...
  describe("._findPackagePath", function () {
    ...
    it("finds linked package.json", function () {
      let result = resolvePackagePath._findPackagePath(
        _cache,
    ...
    it("finds uncle package.json", function () {
      ...
      assert.equal(
        result,
        path.join(dedupped.baseDir, "package.json"),
  ...
  describe("._getRealDirectoryPath", function () {
    ...
    it("resolves linked dir through link to unlinked dir", function () {
      let result = resolvePackagePath._getRealDirectoryPath(
        cache,
        linked.baseDir
      );
      assert.equal(
        result,
        unlinked.baseDir,
        "link should resolve to real path package.json"
      );
      assert.equal(cache.size, 1, "cache should contain 1 entry");
      assert.equal(
        cache.get(linked.baseDir),
        unlinked.baseDir,
        "cached entry from linked path should be to unlinked path"
      );
    });

    it("resolves unlinked dir to itself", function () {
      let result1 = resolvePackagePath._getRealDirectoryPath(
        cache,
        linked.baseDir
      ); // repeat just to load an entry
      let result2 = resolvePackagePath._getRealDirectoryPath(
        cache,
        unlinked.baseDir
      );
      ...
      assert.equal(
        result1,
        result2,
        "2 cache entries should both have the same value (different keys)"
      );
      assert.equal(
        result2,
        unlinked.baseDir,
        "resolving the unlinked path should not change the value"
      );
      assert.equal(
        cache.get(unlinked.baseDir),
        unlinked.baseDir,
        "the cached value for the unlinked path should be to itself"
      );
    });
    ...
    it("resolves an existing file as null (it is not a directory)", function () {
      ...
      assert.isNull(
        cache.get(filePath),
        "cache reference to file should return null"
      );
    });
    ...
    it("resolves a non-existent path as null", function () {
      ...
      assert.isNull(
        result,
        "reference to a non-existent path correctly returns null"
      );
    });
  });
  ...
  describe("._getRealFilePath", function () {
    ...
    it("resolves linked package.json through link to unlinked", function () {
      let result = resolvePackagePath._getRealFilePath(
        cache,
        path.join(linked.baseDir, "package.json")
      );
      assert.equal(
        result,
        path.join(unlinked.baseDir, "package.json"),
        "link should resolve to real path package.json"
      );
      assert.equal(cache.size, 1, "cache should contain 1 entry");
      assert.equal(
        cache.get(path.join(linked.baseDir, "package.json")),
        path.join(unlinked.baseDir, "package.json"),
        "cached entry from linked path should be to unlinked path"
      );
    });
    ...
    it("resolves unlinked package.json to itself", function () {
      let result1 = resolvePackagePath._getRealFilePath(
        cache,
        path.join(linked.baseDir, "package.json")
      ); // repeat just to load an entry
      let result2 = resolvePackagePath._getRealFilePath(
        cache,
        path.join(unlinked.baseDir, "package.json")
      );
      assert.equal(cache.size, 2, "cache should now contain 2 entries");
      ...
      assert.equal(
        cache.get(path.join(unlinked.baseDir, "package.json")),
        path.join(unlinked.baseDir, "package.json"),
        "the cached value for the unlinked path should be to itself"
      );
    });
    ...
    it("resolves an existing directory as null (it is not a file)", function () {
      ...
      assert.isNull(
        cache.get(project.root),
        "cache reference to directory should return null"
      );
    });
  ...
  describe("resolvePackagePath", function () {
    ...
    it("no module name", function () {
      assert.throws(
        () => resolvePackagePath(caches, undefined, "/"),
        TypeError
      );
    });
    ...
    it("baseDir were the end of the path does not exist, but the start does and if resolution proceeds would be a hit", function () {
      assert.equal(
        resolvePackagePath(
          caches,
          "bar",
          path.join(foo.baseDir, "node_modules", "does-not-exist")
        ),
        path.join(foo.baseDir, "node_modules", "bar", "package.json")
      );
    });
    ...
    it("linked directory as name", function () {
      ...
      assert.equal(
        path.join(unlinked.baseDir, "package.json"),
        result,
        'should resolve to unlinked "linked/package.json"'
      );
    });
    ...
    it("through linked directory as dir to node_modules package", function () {
      ...
      assert.equal(
        path.join(unlinked.baseDir, "node_modules", "bar", "package.json"),
        result,
        'should resolve to unlinked "linked/node_ odules/bar/package.json"'
      );
    });
    ...
    it(". relative path ", function () {
      let result = resolvePackagePath(
        caches,
        "..",
        path.join(foo.baseDir, "node_modules")
      );
      assert.equal(
        path.join(foo.baseDir, "package.json"),
        result,
        'should resolve to "foo/package.json"'
      );
    });
